...
const TeacherDashboard = () => {
  ...
  const [timetable] = useState([
    ...
    },
    {
      id: 23,
      day: 'Thursday',
      subject: 'Current Time Test Class',
      class: 'CS-TEST',
      startTime: '17:10',
      endTime: '18:00',
      room: 'Room DEBUG'
  ...
  const getCurrentClass = () => {
    ...
    // If no class found, return a test class for demonstration
    if (!foundClass) {
      console.log('No live class found, showing demo class')
      return {
        id: 999,
        day: currentDay,
        subject: 'Live Test Class - ' + `${currentHour}:${currentMinute.toString().padStart(2, '0')}`,
        class: 'DEMO',
  ...
  const getTodayClasses = () => {
    ...
    return timetable.filter(slot => slot.day === currentDay)
          ...
          <div className="h-full bg-white/8 backdrop-blur-3xl border-r border-white/20 shadow-2xl">
            ...
            <div className="p-6 border-b border-white/20">
              <div className={`transition-all duration-300 ${
                ...
                <Avatar className={`transition-all duration-300 flex-shrink-0 backdrop-blur-sm ${
                  sidebarHovered ? 'w-12 h-12' : 'w-8 h-8'
                }`}>
                  <AvatarFallback className={`bg-white/90 text-black font-bold backdrop-blur-sm ${
                ...
                {navItems.map((item) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full flex items-center rounded-2xl transition-all duration-300 transform hover:scale-105 backdrop-blur-2xl ${
                        sidebarHovered ? 'gap-3 px-4 py-4' : 'justify-center py-4 px-0'
                      } ${
                    ...
                    >
                      ...
                      <span className={`font-medium transition-all duration-300 overflow-hidden whitespace-nowrap ${
                        sidebarHovered ? 'opacity-100 w-auto ml-0' : 'opacity-0 w-0 absolute'
                      }`}>
                        {item.label}
                      </span>
                    </button>
            ...
            <div className="p-6 border-t border-white/10">
              <Button
                variant="ghost"
                size="sm"
                className={`w-full text-white/70 hover:text-white hover:bg-white/15 transition-all duration-300 hover:scale-105 transform rounded-2xl py-3 backdrop-blur-2xl border border-transparent hover:border-white/20 hover:shadow-lg ${
                  sidebarHovered ? 'justify-start px-4' : 'justify-center px-0'
                }`}
                asChild
                title={!sidebarHovered ? 'Sign Out' : ''}
              >
                <Link to="/student-teacher/login" className={`flex items-center w-full ${
